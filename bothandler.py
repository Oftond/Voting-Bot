...
class bothandler:
    ...
    def _register_handlers(self):
        ...
        self.dp.message.register(
            self.handle_choose_poll_to_manage,
            StateFilter(self.PollManagement.choosing_poll)
        )
        self.dp.message.register(
            self.handle_confirm_management,
            StateFilter(self.PollManagement.confirm_action)
        )
    ...
    # Система голосования (доступна всем)
    ...
    async def handle_delete(self, message: types.Message, state: FSMContext):
        ...
        # Для админов показываем все голосования, для обычных пользователей - только свои
        if user_id in config.ADMIN_IDS:
            polls_to_show = self.active_polls
            message_text = "Выберите ID голосования для управления:"
        else:
            polls_to_show = {k: v for k, v in self.active_polls.items() if v['creator_id'] == user_id}
            message_text = "Ваши голосования для управления:"
        ...
        polls_list = "\n".join(
            f"ID: {poll_id} - {poll['title']} (до {poll['end_time'].strftime('%d.%m.%Y %H:%M')})"
            for poll_id, poll in polls_to_show.items()
        )
        ...
        await state.update_data(is_admin=user_id in config.ADMIN_IDS)
    ...
    async def handle_choose_poll_to_manage(self, message: types.Message, state: FSMContext):
        ...
        try:
            ...
            if datetime.now() > poll['end_time']:
                status_msg = "🔴 Голосование уже завершено"
            else:
                status_msg = "🟢 Голосование активно"
        ...
        if not poll:
            ...
            if message.text == "Удалить":
                ...
                if datetime.now() > poll['end_time']:
                    ...
                    del self.active_polls[poll_id]
                    await message.answer(
                        f"Голосование #{poll_id} досрочно завершено.",
                        reply_markup=keyboard.get_start_keyboard()
                    )
    ...
    async def _check_admin_rights(self, message: types.Message) -> bool:
        """Проверка прав администратора (для создания голосований)"""
        if message.from_user.id not in config.ADMIN_IDS:
            await message.answer("⛔ Только администраторы могут создавать голосования!")
            return False
        return True

...
class bothandler:
    ...
    async def handle_delete(self, message: types.Message, state: FSMContext):
        ...
        if not self.active_polls:
            await message.answer("Нет активных голосований для управления.")
            return
        ...
        await message.answer(
            f"Выберите ID голосования для управления:\n\n{polls_list}",
    ...
    async def handle_choose_poll_to_manage(self, message: types.Message, state: FSMContext):
        ...
        try:
            ...
            # Проверка на уже завершенное голосование
            if datetime.now() > poll['end_time']:
                await message.answer(
                    "Это голосование уже завершено. Вы можете:\n"
    ...
    async def handle_confirm_management(self, message: types.Message, state: FSMContext):
        ...
        if message.text == "Удалить":
            ...
            await message.answer(
                f"Голосование #{poll_id} '{poll['title']}' полностью удалено.",
                reply_markup=keyboard.get_start_keyboard()
            )
            ...
            if datetime.now() > poll['end_time']:
                await message.answer(
                    "Это голосование уже было завершено ранее.",
                    reply_markup=keyboard.get_start_keyboard()
                )
            else:
                poll['end_time'] = datetime.now()
                await message.answer(
                    f"Голосование #{poll_id} '{poll['title']}' досрочно завершено.\n"
                    f"Статистика сохраняется.",
                    reply_markup=keyboard.get_start_keyboard()
                )
